...
def process_file(fh: BinaryIO, stop_tag=DEFAULT_STOP_TAG,
                 ...
                 truncate_tags=True, auto_seek=True, extract_thumbnail=True, show_keys=False):
    """
    ...
    if (debug or True) and details:
        ...
        # We need to look inxb the entire file for the XML
        else:
        ...
        if xmp_bytes:
            ...
            #print(f"{lines=}")
            g = Graph()
            g.parse(data='\n'.join(l.replace('DJI Meta Data', 'DJI%20Meta%20Data') for l in lines[2:-1]), format='xml')
            #print(f"RDF\n{g=}")
            for stmt in g:
                #pprint(stmt)
